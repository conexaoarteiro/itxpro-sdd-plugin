...
- Toda tarefa carrega um perfil, e ele é o primeiro token do sufixo, sempre. Vocabulário fechado: `texto | front | back | infra`. Perfil não tem default e não se infere: tarefa sem perfil, ou com palavra fora do vocabulário, bloqueia o despacho dela mesma. O perfil roteia a carga de insumo do implementer e acrescenta ao piso; ele nunca dispensa item do piso. O checklist de pronto mora em `implementer.md`, casa única: você marca o token e não escreve checklist de perfil no plano.
- Toda tarefa declara em `toca:` a superfície de arquivo que ela edita. Caminho de arquivo ou padrão que resolva no repositório, conferido com `git ls-files`. Prosa não é superfície: `toca: repositório, leitura` não casa com arquivo algum e faz a tarefa aparecer disjunta de tudo pelo motivo errado. Tarefa de leitura pura declara os caminhos que lê, ou vai marcada `depende de: Txx`.
- `independente` é afirmação falseável, e quem a falseia primeiro é você. Cruze as superfícies `toca:` par a par antes de escrever a marcação e só marque `independente` com disjunção verificada. Serial é o default: omissão, dúvida ou superfície que cruza significa `depende de: Txx`. Marcar `independente` sem a conferência é desvio, não otimização.
- O interlock dispara pela matéria tocada, nunca pelo rótulo do perfil: `toca:` que alcance `.gitleaks.toml`, `.github/workflows/`, `.claude/settings.json`, `.claude/hooks/` ou `hooks/` nunca recebe `independente`, em qualquer perfil. Esses caminhos formam um conjunto único; serem arquivos diferentes não os torna superfícies disjuntas.
