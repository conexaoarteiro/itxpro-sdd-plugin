...
Classificação: minor, e a classificação foi discutida antes de escrita, porque este caso cai num vão da régua. As duas correções **mudam comportamento**, então patch está fora: a régua diz que patch corrige texto. Major também está fora, e por uma razão mais forte que a classificação: major aqui é a `1.0.0`, que o CHANGELOG da 0.7.0 reservou para uma declaração de estabilidade do framework, e esta versão não a faz.

Entre os dois, minor se sustenta por medição, e a medição é sobre quem lê. O único gatilho de major que poderia disparar é "quebra instrução existente", pela renumeração das linhas de saída do `varre`, declarada abaixo. Os leitores prescritos daquelas linhas são o condutor, que lê prosa, e a suíte do canônico, que acompanhou. **Nenhuma instrução deste pacote manda um agente adotante parsear aquelas linhas por número.** Então nada que alguém foi instruído a depender deixa de resolver. Quem construiu algo por conta própria em cima do número encontra o aviso aqui, que é para isso que a régua existe.

Corrige:

- **A prova de um box fechado é o commit, não o nome do ramo onde ele nasceu** (#158 do canônico). O verbo `boxes` não exigia prova: exigia que o ramo se chamasse `sdd/<NNN>/<Txx>`. Tarefa fechada por qualquer outro caminho reprovava mesmo carregando commit, e a cadeia do release é toda assim, porque quem a executa é o condutor no próprio checkout, sem worktree e sem ramo de despacho. Medido nas duas fatias do canônico que têm tarefa de release fechada: as duas saíam `81` e as três tarefas acusadas carregavam commit. A régua passa a aceitar o commit em qualquer posição da linha, mantendo a forma `parou em <sha>`. Box em `[x]` sem commit nenhum continua saindo `81`.
- **`varre` e `boxes` passam a medir a mesma prova.** Os dois respondiam metades da mesma pergunta com réguas de força diferente, e a mais fraca era a que varre: o `boxes` acusava `[x]` sem prova, e o `varre`, que roda sobre toda fatia entregue na retomada e no fechamento, olhava só `[~]` e `[!]`. Fatia declarada entregue com box fechado sem prova passava silenciosa pelo verbo mais usado. O `varre` ganha a direção, com o mesmo código `81`, e a tarefa do dono fica fora nos dois, como já ficava.
- **A regra da prova passa a morar num lugar só**, na constante `PROVA` do topo do fence, lida pelos dois verbos. A divergência existiu porque a régua estava escrita num verbo e ausente no outro; escrevê-la duas vezes seria plantar a próxima.
- **Dois erros de texto na tabela de códigos de saída do Passo 0**, que se declara casa única. O código `81` ainda descrevia a régua anterior, "não nomeia o ramo dela e um commit". E o código `83` era emitido pelo fence e citado em quatro pontos da prosa sem constar da tabela. Casa única com buraco é casa única que mente.

Muda comportamento: o `varre` passa a poder sair `81`, que antes ele nunca emitia, e quem trata código desconhecido cai no ramo de parar e perguntar, que é o comportamento certo e não uma instrução que deixou de valer. A saída dele ganhou uma linha e **as seguintes foram renumeradas**: `(2)` era `não medido` e passa a ser `box fechado sem prova`; `não medido` vira `(3)` e `marca de ato fora do repositório` vira `(4)`. Quem lê a saída por número lê outra coisa a partir desta versão, e em silêncio; quem a lê por nome não muda nada.

Migração (nota da 0.11.0):
1. Nada a fazer para quem lê a saída dos verbos como prosa, que é o caminho prescrito.
2. Se você automatizou algo em cima de `(2)` ou `(3)` da saída do `varre`, troque o casamento por número pelo nome da direção: `box fechado sem prova`, `não medido` e `marca de ato fora do repositório` não mudaram de texto, só de posição.
3. O `81` do `varre` é código novo nesta versão. Se o seu tratamento enumerava os códigos dele, acrescente-o; se caía no ramo de desconhecido, ele já fazia a coisa certa, que é parar.

Integridade: sha256:d97a3f4211ee477198ac4a072d0c8912f7da7731370f6262e2debeed6feb17c1 (amarração 0.11.0 → hash; conjunto do pacote, excluindo CHANGELOG.md e INTEGRIDADE.txt).
